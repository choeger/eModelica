 ...
 * </p>
 * <p>
 * Current problems: - the size computation is too small - focusLost event is
 * not sent (see https://bugs.eclipse.org/bugs/show_bug.cgi?id=84532)
 * </p>
 * 
 * @since 3.1
 */
...
public class BrowserInformationControl implements IInformationControl,
	...
	public static boolean isAvailable(Composite parent) {
		if (!fgAvailabilityChecked) {
			try {
				if (parent == null)
					parent = EModelicaPlugin.getActiveWorkbenchShell();
				if (parent == null)
	...
	public BrowserInformationControl(Shell parent, int shellStyle, int style,
		...
		// Browser field
		fBrowser = new Browser(composite, style);
	...
	public void setInformation(String content) {
		...
		content = insertStyles(content);

		fBrowser.setText(content);

	}

	private String insertStyles(String content) {
		...
		if (styles != null) {
			...
			if (buffer.indexOf("<body") != -1) {
				...
				styleBuf.insert(0, "<body ");
				styleBuf.append(">");
				buffer.insert(0, styleBuf);
				buffer.append("</body>");
			}
	...
	public void setSize(int width, int height) {
		fShell.setSize(Math.min(width, fMaxWidth), Math.min(height,
						fMaxHeight));
	}
	...
	public Point computeSizeHint() {
		...
		// HTML2TextReader reader = new HTML2TextReader(new
		// StringReader(fInputText), presentation);
		// String text;
		// try {
	...
	public IInformationControlCreator getInformationPresenterControlCreator() {
		return new IInformationControlCreator() {
			...
			public IInformationControl createInformationControl(Shell parent) {
				return new BrowserInformationControl(parent, SWT.H_SCROLL | SWT.V_SCROLL);
			}
		};
	}
